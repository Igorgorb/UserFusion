...
public class UserServiceIT extends AbstractContainerDatabaseTest {
    ...
    public void testGetUsersWithoutMSSqlSource() throws Exception {
        // corrected connection urls and credentials for test's environment
        dataSourceProperties.getDataSources().stream().filter(d -> !d.getStrategy().equals(DatabaseStrategy.SQLSERVER)).forEach(changer);
    ...
    public void testGetUsersWithoutMySqlSource() throws Exception {
        // corrected connection urls and credentials for test's environment
        dataSourceProperties.getDataSources().stream().filter(d -> !d.getStrategy().equals(DatabaseStrategy.MYSQL)).forEach(changer);
    ...
    public void testGetUsersWithoutPostgresqlSource() throws Exception {
        // corrected connection urls and credentials for test's environment
        dataSourceProperties.getDataSources().stream().filter(d -> !d.getStrategy().equals(DatabaseStrategy.POSTGRES)).forEach(changer);
