...
class DataSourceSupplierTest {
    ...
    void testGetJdbcTemplateDefault() {
        ...
        assertSame(mockDataSource, jdbcTemplate.getDataSource(), "The DataSource should match the one provided.");
    }

    @Test
    void testGetJdbcTemplateWithParameters() {
        final JdbcTemplate jdbcTemplate = dataSourceSupplier.getJdbcTemplate(driver, url, username, password);

        assertNotNull(jdbcTemplate, "The JdbcTemplate should not be null.");
        assertNotNull(jdbcTemplate.getDataSource(), "The JdbcTemplate's DataSource should not be null.");
    }
}
